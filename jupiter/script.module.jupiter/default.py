# -*- coding: utf-8 -*-
#-------------------------------------------------------------------------------
# jupiter by Caperucitaferoz based on previous work by:
...
#
# This file is part of jupiter for Kodi
#
# jupiter for Kodi is free software: you can redistribute it and/or modify
...
class OSD(object):
    def __init__(self, stats):
        ...
        # Background
        self.jupiter_background = xbmcgui.ControlImage(x=posX, y=posY, width=window_w, height=window_h,
                                                     filename=os.path.join(runtime_path, 'resources', 'media' , 'background.png'))
        # icono
        self.jupiter_icon = xbmcgui.ControlImage(x=posX + 25, y=posY + 15, width=38, height=28,
                                               filename=os.path.join(runtime_path, 'resources', 'media' , 'acestreamlogo.png'))
        # title
        self.jupiter_title = xbmcgui.ControlLabel(x=posX + 78, y=posY + 13, width=window_w - 10, height=30,
                                                label="jupiter", font=font_max, textColor='0xFFEB9E17')
        # sep
        self.jupiter_sep1 = xbmcgui.ControlImage(x=posX + 5, y=posY + 55, width=window_w - 10, height=1,
                                               filename=os.path.join(runtime_path, 'resources', 'media', 'separator.png'))
        # Stats
        self.jupiter_status = xbmcgui.ControlLabel(x=posX + 25, y=posY + 80, width=window_w - 10, height=30, font=font_min, label=translate(30009) % '')
        self.jupiter_speed_down = xbmcgui.ControlLabel(x=posX + 25, y=posY + 100, width=window_w - 10, height=30, font=font_min, label=translate(30009) % 0)
        self.jupiter_speed_up = xbmcgui.ControlLabel(x=posX + 25, y=posY + 120, width=window_w - 10, height=30, font=font_min, label=translate(30011) % 0)
        self.jupiter_peers = xbmcgui.ControlLabel(x=posX + 25, y=posY + 140, width=window_w - 10, height=30, font=font_min, label=translate(30012) % 0)
        self.jupiter_downloaded = xbmcgui.ControlLabel(x=posX + 25, y=posY + 170, width=window_w - 10, height=30, font=font_min, label=translate(30013) % 0)
        self.jupiter_uploaded = xbmcgui.ControlLabel(x=posX + 25, y=posY + 190, width=window_w - 10, height=30, font=font_min, label=translate(30014) % 0)

        # sep
        self.jupiter_sep2 = xbmcgui.ControlImage(x=posX + 5, y=posY + window_h - 30, width=window_w - 10, height=1,
                                               filename=os.path.join(runtime_path, 'resources', 'media', 'separator.png'))

    def update(self,**kwargs):
        if self.showing:
            status = {'dl':translate(30007), 'prebuf': translate(30008) %(self.stats.progress) + '%'}
            self.jupiter_status.setLabel(translate(30009) % status.get(self.stats.status, self.stats.status))
            self.jupiter_speed_down.setLabel(translate(30010) % self.stats.speed_down)
            self.jupiter_speed_up.setLabel(translate(30011) % self.stats.speed_up)
            self.jupiter_peers.setLabel(translate(30012) % self.stats.peers)
            self.jupiter_downloaded.setLabel(translate(30013) % (self.stats.downloaded // 1048576))
            self.jupiter_uploaded.setLabel(translate(30014) % (self.stats.uploaded // 1048576))

    def show(self):
        self.window.addControl(self.jupiter_background)
        self.window.addControl(self.jupiter_icon)
        self.window.addControl(self.jupiter_title)
        self.window.addControl(self.jupiter_sep1)
        self.window.addControl(self.jupiter_status)
        self.window.addControl(self.jupiter_speed_down)
        self.window.addControl(self.jupiter_speed_up)
        self.window.addControl(self.jupiter_peers)
        self.window.addControl(self.jupiter_downloaded)
        self.window.addControl(self.jupiter_uploaded)
        self.window.addControl(self.jupiter_sep2)
        self.showing = True

    def hide(self):
        self.showing = False
        self.window.removeControl(self.jupiter_background)
        self.window.removeControl(self.jupiter_icon)
        self.window.removeControl(self.jupiter_title)
        self.window.removeControl(self.jupiter_sep1)
        self.window.removeControl(self.jupiter_status)
        self.window.removeControl(self.jupiter_speed_down)
        self.window.removeControl(self.jupiter_speed_up)
        self.window.removeControl(self.jupiter_peers)
        self.window.removeControl(self.jupiter_downloaded)
        self.window.removeControl(self.jupiter_uploaded)
        self.window.removeControl(self.jupiter_sep2)
...
class MyPlayer(xbmc.Player):
    ...
    def playStream(self, stream, title='', iconimage='', plot='', init_time=0.0):
        ...
        art = {'icon': iconimage if iconimage else os.path.join(runtime_path, 'resources', 'media', 'icono_aces_jupiter.png')}
        listitem.setArt(art)
...
def notification_info(*args,**kwargs):
    ...
    #xbmcgui.Dialog().notification('Acestream %s' % transmitter, msg, os.path.join(runtime_path, 'resources', 'media', 'icono_aces_jupiter.png'))
...
def kill_process():
    ...
    if not server.available:
        ...
        xbmcgui.Dialog().notification(HEADING, translate(30035),
                                      os.path.join(runtime_path, 'resources', 'media', 'icono_aces_jupiter.png'))
        return True
    else:
...
def run(item):
    ...
    elif item.action == "listLinks":
        ##Parte custom. Rellenamos un listado de enlaces
...
def multBaseDecode(tx):
    for i in range(0, 10):
        tx = base64.b64decode(tx)
    try:
        return tx.decode("utf8")
    except:
        try:
            return tx.decode("ISO-8859-1")
        except:
            return tx.decode("utf8", "ignore")
